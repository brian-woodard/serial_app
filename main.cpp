...
int main(int, char**)
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Serial", WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);
    ...
    ULONG status;
    ULONG port_numbers[MAX_PORTS];
    ULONG num_ports_found;

    status = GetCommPorts(port_numbers, MAX_PORTS, &num_ports_found);

    printf("%d serial ports found\n", num_ports_found);
    for (ULONG i = 0; i < num_ports_found; i++)
    {
       if (com_port.length() == 0)
       {
          com_port = "COM";
          com_port += std::to_string(port_numbers[i]);
       }
       printf("%s found\n", com_port.c_str());
    }
    ...
    while (!done)
    {
        ...
        if (ImGui::Button(port.IsConnected() ? "Disconnect" : "Connect"))
        {
           ...
           {
              printf("Open %s\n", com_port.c_str());
              CSerialPort::StopBitsType stop = CSerialPort::ONE;

              if (stop_bits == 2)
                 stop = CSerialPort::TWO;

              port.Open(com_port.c_str(), data_bits, stop, (CSerialPort::ParityType)parity, baud);
              send = false;
        ...
        // serial settings
        char port_str[128];
        strcpy(port_str, com_port.c_str());
        if (ImGui::InputText("Port", port_str, IM_ARRAYSIZE(port_str)))
        {
           com_port = port_str;
        }

        ImGui::InputScalar("Speed (baud)", ImGuiDataType_U32, &baud);
        ...
        if (ImGui::InputInt("Stop bits", &stop_bits))
        {
           if (stop_bits > 2)
              stop_bits = 2;
           else if (stop_bits < 1)
        ...
        // read/write serial at 10 Hz
        frames++;
        if (frames % 6 == 0 && port.IsConnected())
        {
           if (send)
           {
              port.Write((char*)brt_cmd, sizeof(brt_cmd));
              send = false;

              printf("Wrote brightness cmd (%d bytes):\n", sizeof(brt_cmd));
              for (int i = 0; i < sizeof(brt_cmd); i++)
              {
           ...
           char buffer[256];
           int  bytes_read = port.Read(buffer, sizeof(buffer));
